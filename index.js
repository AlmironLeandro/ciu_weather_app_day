  ...
  fetch(url)
    .then(respuesta => respuesta.json())
      .then(resultado => {
        ...
        document.getElementById("diaSiguiente1").innerHTML = `<p>${new Date(resultado.daily[1].dt).toString().substr(0,10)}</p>`
        document.getElementById("diaSiguiente2").innerHTML = `<p>${new Date(resultado.daily[2].dt).toString().substr(0,10)}</p>`
        document.getElementById("diaSiguiente3").innerHTML = `<p>${new Date(resultado.daily[3].dt).toString().substr(0,10)}</p>`
      })}
